...
try{
	...
	$query = "CREATE TABLE IF NOT EXISTS section(
		...
		begin INTEGER NOT NULL COMMENT '改行年',
	...
	$sth = $pdo -> prepare('INSERT INTO section (`id`, `rint`, `lin`, `opc`, `rfid`, `begin`, `end`) VALUES (?, ?, ?, ?, ?, ?, ?)');
	foreach ($contents["ksj_RailroadSection2"] as $v) {
		...
		$sth -> bindValue(6, (int) $v["ksj_exp"]["gml_TimePeriod"]["gml_beginPosition"], PDO::PARAM_INT);
		$sth -> bindValue(7, (int) $v["ksj_exp"]["gml_TimePeriod"]["gml_endPosition"], PDO::PARAM_INT);
		$sth -> execute();
