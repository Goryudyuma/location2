...
if (is_numeric($_REQUEST['x'])&&is_numeric($_REQUEST['y'])) {
	...
	$sth = $pdo->prepare("SELECT stn ,section.lin AS linename, X(pos) AS X,Y(pos) AS Y, GLength(ST_GeomFromText(CONCAT(:geo , X(pos), ' ', Y(pos), ')')))*111.319 AS distance FROM station INNER JOIN (SELECT DISTINCT rfid, lin FROM section WHERE end = 9999) AS section ON section.rfid = sectionid WHERE end = 9999 ORDER BY distance LIMIT 20; ");
