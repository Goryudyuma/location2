...
if (is_numeric($_REQUEST['x'])&&26.0<$_REQUEST['x']&&$_REQUEST['x']<46.0&&is_numeric($_REQUEST['y'])&&127.0<$_REQUEST['y']&&$_REQUEST['y']<146.0) {
	...
	$sth = $pdo->prepare("SELECT section.lin AS linename, section.rfid, section.opc AS opc, ST_DISTANCE(c.pos,point(:north , :east)) AS dist, sectionid FROM (SELECT * FROM curve WHERE intersects(pos, buffer(point(:north , :east), :length))) AS c INNER join (SELECT DISTINCT id, lin, opc, rfid FROM section WHERE end = 9999) AS section ON section.id = c.sectionid ORDER BY dist LIMIT 20;");
